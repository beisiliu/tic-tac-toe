...
int main(int argc, char* argv[])
{
    if ( init("example06", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, SCREEN_HEIGHT, SCREEN_WIDTH, SDL_WINDOW_SHOWN) )
    {
        ...
        while(isRunning)
        {
            ...
            topRect.y = 0;
            topRect.h = imgHeight;
            topRect.w = imgWidth * 2;

            // 下方左右两边各方原图
            SDL_Rect leftDownRect;
            leftDownRect.x = 0;
            leftDownRect.y = 300;
            leftDownRect.h = imgHeight;
            leftDownRect.w = imgWidth;

            SDL_Rect rightDownRect;
            rightDownRect.x = 300;
            rightDownRect.y = 300;
            rightDownRect.h = imgHeight;
            rightDownRect.w = imgWidth;

            // 设置背景色
            SDL_SetRenderDrawColor(gRenderer, 255, 255, 255, 255);
            SDL_RenderClear(gRenderer);
            
            // 每设完一个viewport, 就需要进行一次RenderCopy
            SDL_RenderSetViewport(gRenderer, &topRect);
            SDL_RenderCopy(gRenderer, gTexture, nullptr, nullptr);

            SDL_RenderSetViewport(gRenderer, &leftDownRect);
            SDL_RenderCopy(gRenderer, gTexture, nullptr, nullptr);

            SDL_RenderSetViewport(gRenderer, &rightDownRect);
            SDL_RenderCopy(gRenderer, gTexture, nullptr, nullptr);
